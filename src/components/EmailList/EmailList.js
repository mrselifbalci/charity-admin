...
import {COLUMNS} from './EmailListTableData'
...
export default function EmailList({apiBaseUrl}) {
    ...
    const handleSubmit=(newsId)=>{
        ...
        axios.put(`${apiBaseUrl}/emaillist/${newsId}`,updatedNews)
        .then(res=>{
    ...
    const editNews=async (newsId)=>{
       await  axios
        .get(`${apiBaseUrl}/emaillist/${newsId}`)
        .then((res) => { 
    ...
    const deleteNews=(newsId)=>{
        axios
        .delete(`${apiBaseUrl}/emaillist/${newsId}`)
        .then((res) => {
            window.location.reload()
        })   
        .catch((err) => {
