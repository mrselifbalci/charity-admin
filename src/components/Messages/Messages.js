...
import {COLUMNS} from './MessagesTableData'
...
export default function Messages({apiBaseUrl}) {
    ...
    const handleSubmit=(newsId)=>{
        ...
        axios.put(`${apiBaseUrl}/messages/${newsId}`,updatedNews)
        .then(res=>{
    ...
    const editNews=async (newsId)=>{
       await  axios
        .get(`${apiBaseUrl}/messages/${newsId}`)
        .then((res) => { 
            console.log(res.data.response.quoteAuthorMedia) 
    ...
    const deleteNews=(newsId)=>{
        axios
        .delete(`${apiBaseUrl}/messages/${newsId}`)
        .then((res) => {
            window.location.reload()
        })   
        .catch((err) => {
