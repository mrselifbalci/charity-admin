...
import { COLUMNS } from './AmbassadorsData';
import '../../../styles/table.css';
import './Ambassadors.css';

Modal.setAppElement('#root');

const Ambassadors = ({ apiBaseUrl }) => {
	...
	const viewAmbassadors = async (donationId) => {
		await axios
			...
			.then((res) => {
				...
				setReasonToJoin(res.data.data.reason_to_join);
	...
	const deleteAmbassador = (donationId) => {
		axios
	...
	useEffect(() => {
		axios
			.get(`${apiBaseUrl}/donations/type/ambassador`)
			.then((res) => {
				...
				<Modal
					...
					<div className='modal-container'>
						...
						<form className='modal-form'>
							...
							<div className='modal-form-item'>
								<h2 className='modal-form-item-title'>Reason to Join</h2>
								<p className='modal-form-item-content'>{reasonToJoin}</p>
							</div>
							<div className='modal-form-item'>
							...
							<div className='modal-form-item'>
								<h2 className='modal-form-item-title'>Comments</h2>
								...
								<tr className='table-row-wrapper' {...row.getRowProps()}>
									...
									<td className='table-action-icons-wrapper'>
										<BsFillEyeFill
											...
											onClick={() => {
												viewAmbassadors(row.original._id);
											}}
										/>
										&nbsp; &nbsp;
										<BsFillTrashFill
											...
											onClick={() => {
												deleteAmbassador(row.original._id);
											}}
										/>
									</td>
								</tr>
